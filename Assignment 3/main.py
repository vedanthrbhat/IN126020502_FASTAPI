...
## Updating the price of particular category
@app.put('/products/discounts')
def apply_category_discount(
    category: str = Query(...,description="Category for which discount need to apply"),
    discount_percentage: float = Query(...,description="Discount percentage")
):
    updated_product=[]
    for product in products:
        if(product['category'].lower()==category.lower()):
            product['price'] = round(product['price'] - product['price']*(discount_percentage/100),2)
            updated_product.append(product)

    if not updated_product: 
        return {'message': f'No products found in category: {category}'}
    return { 'message': f'{discount_percentage}% discount applied to {category}', 'updated_count': len(updated_product), 'updated_products': updated_product }
...
def get_product_audit():
    ...
    for product in products:
        if(product['in_stock']==True):
            ...
            out_stock_list_name.append(product['name'])
    return {'Total Products':total_products,"In stock count":in_stock_count,"Out of stock names":out_stock_list_name,"Total stock value":total_stock_value*10,"Most expeensive":{'name':most_expensive['name'],'price':most_expensive['price']}}
